...
for n=1:nfolder
    ...
    for i=1:numel(GFPfile)
        ...
        flat_lipid_bkg=mean(mean([Lipidimage(i).rawintensity(1:15,1:51) Lipidimage(i).rawintensity(37:51,1:51)]));
        flat_GFP_bkg=mean(mean([GFPimage(i).rawintensity(1:15,1:51) GFPimage(i).rawintensity(37:51,1:51)]));
    ...
    %rescale 2-6 and 6-10 images to same baseline
    %600nm bar area

    %Lipid_ctl_s_area=Lipidimage(5).bkgdsub(20:32,12:39);
    %Lipid_ctl_l_area=Lipidimage(6).bkgdsub(20:32,13:40);
    %GFP_ctl_s_area=GFPimage(5).bkgdsub(20:32,12:39);
    %GFP_ctl_l_area=GFPimage(6).bkgdsub(20:32,13:40);

    %Lipid_Diff_t=sum(sum(Lipid_ctl_l_area))/sum(sum(Lipid_ctl_s_area));
    %GFP_Diff_t=sum(sum(GFP_ctl_l_area))/sum(sum(GFP_ctl_s_area));

    %for i=1:5

        %Lipidrescale=times(Lipidimage(i).bkgdsub,Lipid_Diff_t);
        %Lipidimage(i).rescale=Lipidrescale;
        %GFPrescale=times(GFPimage(i).bkgdsub,GFP_Diff_t);
        %GFPimage(i).rescale=GFPrescale;

    %end

    %for i=6:10

        %Lipidimage(i).rescale=Lipidimage(i).bkgdsub;
        %GFPimage(i).rescale=GFPimage(i).bkgdsub;

    %end

    %rescale image compared to the flat surface peak of 1000nm nanobar
    ...
    for i=1:numel(GFPfile)
        ...
        flat_lipid_nor_bkg=mean(mean([Lipidimage(i).normalization(1:15,1:51) Lipidimage(i).normalization(37:51,1:51)]));
        flat_GFP_nor_bkg=mean(mean([GFPimage(i).normalization(1:15,1:51) GFPimage(i).normalization(37:51,1:51)]));
