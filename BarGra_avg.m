...
% 1.4. For all masks and related peaks file (ngood point)

    % Setup blank average images for all 10 diameters
    bar_bucket = zeros(10, 1);
    %bar_avg_BF = zeros(masksize, masksize, 10);   
...
%   2.1 Loop all Mask file in the same folder one by one
    ...
    %   2.3 Loop through each mask /bar postion
            ...
            for p = 1:NumPoints
                ...
                xpos = MaskPoints(2,p);
                ypos = MaskPoints(1,p);
                barcode = ceil(MaskPoints(3,p)/2); % rounds each element of X to the nearest integer greater than or equal to that element.
                ...
                if xpos>=halfrange+1 && xpos<=ysize-halfrange && ypos>=halfrange+1 && ypos<=xsize-halfrange  %???
                   ...
                   gfpArea=GFPimage_subBT(xpos-halfrange:xpos+halfrange,ypos-halfrange:ypos+halfrange);
